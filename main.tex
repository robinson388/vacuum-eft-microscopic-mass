...
% =============================================================
We employ the effective infrared fixed-point value 
$\alpha_s \to 1.0$ in the non-perturbative regime, 
consistent with standard hadronic string-tension 
approximations and infrared freezing constraints. 
In this context, $\sigma_{\rm eff}$ represents the 
universal vacuum response substrate before scaling.

The physical equilibrium configuration of the localized 
soliton core is determined by minimizing the total 
energy functional with respect to the radial boundary 
coordinate: $dE_{\rm tot}/dr = 0$. Evaluating this 
variational constraint at the stable matching surface 
yields the equilibrium Compton radius $r_\star$:
\begin{equation}
\label{eq:compton_minimization}
\left. \frac{dE_{\rm tot}}{dr} \right|_{r=r_\star} = 0 
\implies r_\star = \left( \frac{\alpha\hbar c}{8\pi\sigma_{\rm eff}} 
\right)^{1/3} \equiv \frac{\hbar}{m_e c}.
\end{equation}

By solving Eq.~\eqref{eq:compton_minimization} explicitly 
for the surface parameter, the effective holographic tension 
is structurally locked to the electron mass ground state:
\begin{equation}
...
This effective IR freezing of the strong coupling 
is formally mapped onto the non-perturbative 
conformal window of QCD, matching the long-distance 
limits derived from functional Schwinger-Dyson 
equations and light-front holographic frameworks. 
Furthermore, the geometric packing factor $r_p = 4\lambda_c$ 
emerges naturally as a topological consequence 
of the boundary embedding. In a localized $N_c=3$ 
color-singlet state, the spatial projection of the 
compactified $\mathcal{S}^2$ vacuum interface 
introduces a mandatory wrapping number correction 
($1 + N_c = 4$). This structural scale separation 
insulates the core from bare constituent mass 
divergences, ensuring that the total baryonic mass 
is localized strictly as a coherent self-sustained 
...
Zenodo DOI: \texttt{10.5281/zenodo.21540551}.


\begin{acknowledgments}
The author expresses sincere gratitude to the open-source 
scientific computing community for providing the numerical 
infrastructure utilized throughout this research, specifically 
the Python Core development team, NumPy, and Matplotlib 
libraries. Special recognition is extended to the Linux 
subsystem maintenance teams for supporting the robust 
computational environment required for the validation pipelines, 
as well as the developers of the REVTeX editorial framework 
for enabling rigorous publication-grade manuscript integration.
\end{acknowledgments}
